...
export function AuthFormCard({ mode, nextPath = "" }: AuthFormCardProps): ReactElement {
    ...
    try {
      if (mode === "register") {
        const response = await fetch("/api/auth/register", {
          ...
          body: JSON.stringify({ name: String(formData.get("name") ?? "").trim(), email, password, role: String(formData.get("role") ?? "JAMAAH") }),
        });
        ...
        router.push({ pathname: "/verify-email", query: safeNextPath ? { email, next: safeNextPath } : { email } });
        return;
      }
      ...
      if (isLoginSuccessResponse(payload)) {
        router.push(getPostLoginPath(payload.redirectTo, safeNextPath));
        return;
      }
  ...
  return (
    <section className="w-full rounded-[12px] border border-[var(--border)] bg-white/92 p-4 shadow-[0_18px_54px_rgba(21,35,29,0.11)] backdrop-blur sm:p-5 lg:p-6">
      <div className="mb-4 border-l-2 border-[var(--gold)] pl-3 lg:mb-5 lg:pl-4">
        ...
        <h1 className="mt-1.5 text-[1.55rem] font-black leading-[1.05] tracking-[-0.035em] text-[var(--charcoal)] sm:text-[1.78rem] lg:text-[1.95rem]">{copy.title}</h1>
      ...
      <form className="grid gap-2.5 lg:gap-3" onSubmit={handleSubmit}>
        ...
        {errorMessage ? <p className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm font-bold leading-6 text-[var(--error)]" role="alert">{errorMessage}</p> : null}
        <button type="submit" disabled={isSubmitting} className="mt-1 flex h-11 w-full items-center justify-center rounded-[9px] bg-[var(--emerald)] text-sm font-extrabold text-white shadow-[0_10px_22px_rgba(27,107,74,0.2)] transition duration-200 hover:bg-[var(--emerald-light)] focus:outline-none focus:ring-4 focus:ring-[var(--emerald)]/20 disabled:cursor-not-allowed disabled:opacity-60 lg:h-12">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-[var(--border)] pt-3 text-xs font-bold text-[var(--text-muted)]">
        <Link href={safeNextPath ? `${copy.alternateHref}?next=${encodeURIComponent(safeNextPath)}` : copy.alternateHref} className="text-[var(--emerald)] underline-offset-4 hover:underline">{copy.alternateLabel}</Link>
        {mode === "login" ? <Link href="/forgot-password" className="underline-offset-4 hover:text-[var(--emerald)] hover:underline">{t("forgotPassword")}</Link> : null}
      </div>
    </section>
  );
}
...
function FieldFrame({ children, htmlFor, icon, label }: FieldFrameProps): ReactElement {
  return <div className="grid gap-1"><label className="text-[0.73rem] font-extrabold text-[var(--charcoal)]" htmlFor={htmlFor}>{label}</label><div className="relative"><div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">{icon}</div>{children}</div></div>;
}
...
function getPostLoginPath(roleRedirectTo: string, safeNextPath: string): string {
  if (!safeNextPath) return roleRedirectTo;
  return safeNextPath.startsWith(roleRedirectTo) ? safeNextPath : roleRedirectTo;
}
