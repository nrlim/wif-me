...
type MobileNavItem = { readonly href: string; readonly labelKey: string; readonly icon: LucideIcon };

const ADMIN_PRIMARY_ITEMS: readonly MobileNavItem[] = [
...
const ADMIN_SHEET_ITEMS: readonly MobileNavItem[] = [
  { href: "/admin", labelKey: "roles.admin.overview", icon: House },
...
const JAMAAH_PRIMARY_ITEMS: readonly MobileNavItem[] = [
  { href: "/jamaah", labelKey: "roles.jamaah.overview", icon: House },
...
const JAMAAH_SHEET_ITEMS: readonly MobileNavItem[] = [
  { href: "/jamaah", labelKey: "roles.jamaah.overview", icon: House },
  { href: "/jamaah/search", labelKey: "roles.jamaah.search", icon: Search },
  { href: "/jamaah/bookings", labelKey: "roles.jamaah.bookings", icon: CalendarDays },
  { href: "/jamaah/payments", labelKey: "roles.jamaah.payments", icon: CreditCard },
...
export function DashboardMobileNav(): ReactElement | null {
  ...
  return (
    <>
      <nav aria-label={t("mobileNav")} className="fixed inset-x-4 bottom-[calc(0.8rem+env(safe-area-inset-bottom))] z-50 flex h-[70px] items-center rounded-[18px] border border-white/70 bg-white/88 px-2 shadow-[0_14px_36px_rgba(22,33,28,0.14)] backdrop-blur-xl md:hidden">
        {config.primary.map((item) => item.href === "CENTER" ? (
          <div key={item.href} className="relative flex h-full w-[76px] items-center justify-center">
            <button type="button" aria-label={t("openMenu")} onClick={() => setIsOpen(true)} className="absolute -top-5 flex size-16 items-center justify-center rounded-full border-[5px] border-[var(--ivory)] bg-[var(--emerald)] text-white shadow-[0_12px_28px_rgba(27,107,74,0.34)] active:scale-95">
              <item.icon className="size-7" aria-hidden="true" />
            </button>
          </div>
        ) : <MobileTab key={item.href} item={item} pathname={pathname} label={t(item.labelKey)} />)}
      </nav>

      <button type="button" aria-label={t("closeMenu")} className={cn("fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm md:hidden", isOpen ? "block" : "hidden")} onClick={closeSheet} />
      <section aria-label={t("mainMenu")} className={cn("fixed inset-x-0 bottom-0 z-[70] rounded-t-[28px] bg-white px-5 pb-[calc(2rem+env(safe-area-inset-bottom))] pt-4 shadow-[0_-18px_45px_rgba(22,33,28,0.18)] transition-transform duration-300 md:hidden", isOpen ? "translate-y-0" : "translate-y-full")}>
        <div className="mx-auto mb-5 h-1.5 w-12 rounded-full bg-[var(--border)]" />
        <div className="mb-5 flex items-center justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-[0.14em] text-[var(--gold)]">{t("mainMenu")}</p><h2 className="mt-1 text-xl font-extrabold text-[var(--charcoal)]">{t(config.titleKey)}</h2></div><button type="button" onClick={closeSheet} className="flex size-11 items-center justify-center rounded-full bg-[var(--ivory)] text-[var(--text-muted)]"><X className="size-5" aria-hidden="true" /></button></div>
        <div className="grid grid-cols-2 gap-3">{config.sheet.map((item) => <Link key={item.href} href={item.href} onClick={closeSheet} className="rounded-2xl border border-[var(--border)] bg-[var(--ivory)] p-4"><item.icon className="size-5 text-[var(--emerald)]" aria-hidden="true" /><p className="mt-3 text-sm font-extrabold text-[var(--charcoal)]">{t(item.labelKey)}</p></Link>)}</div>
        <Link href="/" className="mt-4 flex min-h-12 items-center justify-center gap-2 rounded-2xl border border-red-100 bg-red-50 text-sm font-extrabold text-red-700"><LogOut className="size-5" aria-hidden="true" />{t("logout")}</Link>
      </section>
    </>
  );
}
