...
import { updateMuthawifPasswordAction } from "@/app/[locale]/(dashboard)/muthawif/actions";
import { requireRoleSession } from "@/lib/auth/current-session";

export const metadata: Metadata = { title: "Change Password Muthawif" };

type MuthawifSecurityPageProps = {
  readonly searchParams: Promise<{ readonly notice?: string }>;
};

export default async function MuthawifSecurityPage({ searchParams }: MuthawifSecurityPageProps): Promise<ReactElement> {
  await requireRoleSession([UserRole.MUTHAWIF]);
  const [t, params] = await Promise.all([getTranslations("Partner.muthawif.profile"), searchParams]);
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[1fr_340px]">
      <section className="rounded-xl border border-[var(--border)] bg-white p-4 md:p-5">
        ...
        <form action={updateMuthawifPasswordAction} className="mt-6 grid gap-5">
