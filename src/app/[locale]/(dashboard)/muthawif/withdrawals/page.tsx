...
export const metadata: Metadata = { title: "Penarikan Muthawif" };

type PageProps = { readonly params: Promise<{ readonly locale: string }>; readonly searchParams: Promise<{ readonly notice?: string }> };

export default async function MuthawifWithdrawalsPage({ params, searchParams }: PageProps): Promise<ReactElement> {
  const session = await requireRoleSession([UserRole.MUTHAWIF]);
  const [{ locale }, query, t, common, data] = await Promise.all([params, searchParams, getTranslations("Partner.muthawif.withdrawals"), getTranslations("Partner.common"), getPartnerWithdrawalData(session.userId)]);
