...
export const metadata: Metadata = { title: "Pendapatan Provider" };

export default async function ProviderEarningsPage(): Promise<ReactElement> {
  const session = await requireRoleSession([UserRole.PROVIDER]);
  const [t, common, statusT, dashboard, rows] = await Promise.all([
    getTranslations("Partner.provider.earnings"),
