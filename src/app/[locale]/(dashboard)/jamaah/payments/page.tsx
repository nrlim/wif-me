...
function PaymentTableRow({ payment, statusT, actionLabel }: { readonly payment: JamaahPaymentRow; readonly statusT: (key: string) => string; readonly actionLabel: string }): ReactElement {
  return <tr><td className="px-5 py-4"><p className="font-extrabold text-[var(--charcoal)]">{payment.orderTitle}</p><p className="mt-1 text-xs font-bold text-[var(--text-muted)]">{formatDate(payment.createdAt)}</p></td><td className="px-5 py-4 font-extrabold text-[var(--charcoal)]">{payment.reference}</td><td className="px-5 py-4 font-extrabold text-[var(--charcoal)]">{payment.amount}</td><td className="px-5 py-4"><StatusBadge label={statusT(`payment.${payment.status}`)} variant={paymentStatusVariant(payment.status)} /></td><td className="px-5 py-4"><Link href={`/jamaah/payments/${payment.id}`} className="inline-flex min-h-11 items-center rounded-lg border border-[var(--border)] px-4 font-extrabold text-[var(--charcoal)]">{actionLabel}</Link></td></tr>;
}
