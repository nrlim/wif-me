...
type JamaahServiceDetailPageProps = {
  readonly params: Promise<{ readonly category: string; readonly locale: string }>;
  readonly searchParams: Promise<Record<string, string | string[] | undefined>>;
};

export async function generateMetadata({ params }: JamaahServiceDetailPageProps): Promise<Metadata> {
  const { category, locale } = await params;
  const detail = await getPublicCategoryDetail(category);
  if (!detail) return { title: "Cari Layanan" };
  const t = await getTranslations({ locale, namespace: "Services" });
  return { title: t("detailMeta", { category: t(`categories.${detail.key}.title`) }) };
}

export default async function JamaahServiceDetailPage({ params, searchParams }: JamaahServiceDetailPageProps): Promise<ReactElement> {
  ...
  const [t, categories, detail, locations] = await Promise.all([getTranslations("Services"), getPublicCategoriesSummary(), getPublicCategoryDetail(category, filters), getPublicLocations()]);
  if (!detail) notFound();
  ...
  return (
    <div className="flex flex-col gap-4 lg:gap-8">
      ...
      {/* Content layout */}
      <div className="grid gap-4 lg:grid-cols-[260px_minmax(0,1fr)] lg:items-start lg:gap-x-8 lg:gap-y-0 lg:h-[calc(100vh-144px)] lg:overflow-hidden">
        {/* Title (Row 1, Col 1) */}
        ...
        <div className="flex flex-col gap-4 lg:col-start-1 lg:row-start-2 lg:h-full lg:overflow-y-auto lg:pr-2 lg:pb-8 lg:scrollbar-none lg:gap-6">
          <PublicServiceCategoryNav ariaLabel={t("categoryNavLabel")} basePath="/jamaah/search" activeSlug={detail.slug} asSidebar items={categories.map((item) => ({ key: item.key, slug: item.slug, label: t(`categories.${item.key}.title`), count: item.serviceCount }))} />
        ...
        <div className="flex flex-col gap-4 lg:col-start-2 lg:row-start-2 lg:h-full lg:overflow-y-auto lg:pl-1 lg:pb-8">
          {/* Results count — mobile */}
          <p className="mb-1 text-xs font-bold text-[var(--text-muted)] lg:hidden">{resultSummary}</p>
