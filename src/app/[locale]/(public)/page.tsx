  ...
  return (
    <>
      ...
      <main className="min-h-dvh overflow-hidden bg-[var(--ivory)] pb-20 text-[var(--charcoal)] min-[900px]:pb-0">
        ...
        <section id="bergabung" className="relative overflow-hidden bg-gradient-to-br from-[var(--charcoal)] to-[#0d2a26] py-[clamp(4rem,8vw,6rem)]"><div className="relative z-10 mx-auto w-full max-w-[1400px] px-5 sm:px-8 lg:px-10"><div className="mb-12 text-center"><div className="mb-4 inline-block rounded-full bg-white/10 px-4 py-1.5 text-[0.75rem] font-bold uppercase tracking-[0.1em] text-[var(--emerald-light)]">{t("join.eyebrow")}</div><h2 className="mb-4 text-[clamp(1.75rem,4vw,2.5rem)] font-extrabold tracking-[-0.02em] text-white">{t("join.title")}</h2><p className="mx-auto max-w-[560px] text-[0.9375rem] leading-[1.7] text-white/60">{t("join.description")}</p></div><div className="grid grid-cols-1 gap-8 md:grid-cols-2"><JoinCard title={joinCards[0]?.title ?? ""} desc={joinCards[0]?.desc ?? ""} button={t("join.button")} href="/register" icon={<User className="size-7 text-white" />} /><JoinCard title={joinCards[1]?.title ?? ""} desc={joinCards[1]?.desc ?? ""} button={t("join.button")} href="/register" icon={<BriefcaseBusiness className="size-7 text-white" />} gold /></div></div></section>
